...
const listEntriesByAccountAndTime = `-- name: ListEntriesByAccountAndTime :many
SELECT id, account_id, amount, created_at FROM entries
WHERE account_id = $1
AND created_at >= $2
AND created_at <= $3
...
type ListEntriesByAccountAndTimeParams struct {
	AccountID   int64     `json:"account_id"`
...
func (q *Queries) ListEntriesByAccountAndTime(ctx context.Context, arg ListEntriesByAccountAndTimeParams) ([]Entry, error) {
	rows, err := q.db.QueryContext(ctx, listEntriesByAccountAndTime,
		arg.AccountID,
		arg.CreatedAt,
		arg.CreatedAt_2,
		arg.Limit,
